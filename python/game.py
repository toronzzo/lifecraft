...
class Animal(Organism):
    def move(self, board):
        ...
        if direction == 'up' and self.y > 0:
            self.y -= 1
        elif direction == 'down' и self.y < len(board) - 1:
            self.y += 1
...
class GameBoard:
    ...
    def display(self, screen, cell_size):
        ...
        for organism in self.organisms:
            ...
            pygame.draw.rect(screen, color, pygame.Rect(organism.x * cell_size, organism.y * cell_size, cell_size, cell_size))
            font = pygame.font.SysFont(None, 24)
            text = font.render(organism.name[0], True, (255, 255, 255))
            screen.blit(text, (organism.x * cell_size + 5, organism.y * cell_size + 5))
        pygame.display.flip()
